...
func (r *SStReader) ReadFooter() error {
	...
	// Create a buffer to read the values
...
func DecodeBlock(block []byte) ([]byte, []int, error) {
	if len(block) < 4 {
		return nil, nil, fmt.Errorf("block too small")
	}
	...
	// Validate restart points size
	if nRestartPoint*4+4 > n {
		return nil, nil, fmt.Errorf("invalid restart points count")
	}

	// Calculate positions
	oRestartPoint := n - (nRestartPoint * 4) - 4
	if oRestartPoint < 0 {
		return nil, nil, fmt.Errorf("invalid restart point offset")
	}
	...
	for i := 0; i < nRestartPoint; i++ {
		...
		if restartPoint[i] > oRestartPoint {
			return nil, nil, fmt.Errorf("invalid restart point value")
		}
	}
...
func ReadIndex(index []byte) []*Index {
	...
	for {
		...
		if err != nil {
			panic(errors.New("error in readRecord(prvKey,indexBuf) : " + err.Error()))
			break
		}
...
func NewSStWriter(file string, conf *config.Config) (*SsWriter, error) {
	fd, err := os.OpenFile(path.Join(conf.Dir, file), os.O_WRONLY|os.O_CREATE, 0777)
	if err != nil {
...
func (w *SsWriter) Finish() (int64, map[uint64][]byte, []*Index, error) {
	...
	// Write filter blocks and record filter offset
	...
	// Write index blocks and record index offset
	...
	if err := w.verifyFooter(); err != nil {
		log.Printf("Footer verification failed: %v", err)
	}
